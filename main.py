...
def login():
    ...
    if userLogin not in managers:
        ...
        print('It is currently:'+ y.strftime('%I'+':'+'%M'+'%p '+ ''+ '%m'+'-'+'%Y'))
        isManager = True
    return isManager, userLogin


def authorizedUser(im):
    if im:
        main()
    else:
        print('Unable to authorize user')
        print('Please contact a sys admin')
...
def lossFile(eD):
    ...
    output.close()
    viewReport = input(
        'Would you like to view the data stored in the report? Enter (y/n): \n')
...
def doAnother(u):
    ...
    while again == False:
        ...
        if again == 'y' or again == 'yes':
            ...
            report = lossReport(shipmentValue, itemCost,markdownPercentage, message)
...
def main():
    ...
    if access:
        ...
        reportOut = readfile(outF)
        again = doAnother(user)
    else:
        print('you are not authorized to make markdowns')
